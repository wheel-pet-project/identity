...
public class PasswordRecoverTokenRepository(IDbConnection connection, PostgresRetryPolicy retryPolicy) 
    : IPasswordRecoverTokenRepository
{
    public async Task<Result> AddPasswordRecoverToken(
...
VALUES (@accountId, @resetPasswordTokenHash, false, @expiresIn);";
    ...
    public async Task<Result<(string ResetPasswordTokenHash, 
        ...
        DateTime ExpiresIn)>> GetPasswordRecoverToken(Guid accountId)
    {
        var sql = @"
...
LIMIT 1";
    ...
    public async Task<Result> MarkPasswordRecoverTokenAsApplied(Guid accountId)
    {
        var sql = @"
