...
public class KafkaProducer(
    ITopicProducer<string, SendConfirmationEmailCommand> sendConfirmationEmailProducer,
    ITopicProducer<string, SendRecoverPasswordEmailCommand> passwordRecoverTokenCreatedProducer) 
    : IMessageBus
{
    public async Task Publish(ConfirmationTokenCreatedDomainEvent domainEvent, CancellationToken cancellationToken)
    {
        await sendConfirmationEmailProducer.Produce(key: domainEvent.EventId.ToString(),
            new SendConfirmationEmailCommand(domainEvent.Email, "someurl:" + domainEvent.ConfirmationToken),
            cancellationToken);
    }
    ...
    public async Task Publish(PasswordRecoverTokenCreatedDomainEvent domainEvent, CancellationToken cancellationToken)
    {
        await passwordRecoverTokenCreatedProducer.Produce(key: domainEvent.EventId.ToString(),
            new SendRecoverPasswordEmailCommand(domainEvent.Email, "someurl:" + domainEvent.RecoverToken),
            cancellationToken);
    }
}