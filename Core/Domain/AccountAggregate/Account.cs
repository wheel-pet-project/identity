...
public class Account : Aggregate
{
    ...
    {
        if (!ValidateEmail(newEmail)) throw new ValueIsInvalidException($"{nameof(newEmail)} cannot be invalid or null");
        Email = newEmail;
    }
    
    public void SetPhone(string newPhone)
    {
        if (!ValidatePhone(newPhone)) throw new ValueIsInvalidException($"{nameof(newPhone)} cannot be invalid phone or null");
        Phone = newPhone;
    }
